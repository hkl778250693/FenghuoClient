...
import com.fenghks.business.print.params.BarCode;
import com.fenghks.business.print.params.Constant;
import com.fenghks.business.print.params.Goods;
import com.fenghks.business.print.params.Image;
import com.fenghks.business.print.params.PosParam;
import com.fenghks.business.print.params.PosTpl;
import com.fenghks.business.print.params.QrCode;
import com.fenghks.business.print.params.Text;
...
public class EscPos {
     ...
     * @throws IOException
     */
    public EscPos(OutputStream outputStream, String encoding) throws IOException {
    ...
    public synchronized static EscPos getInstance(OutputStream outputStream, String encoding) throws IOException {
        escPos = new EscPos(outputStream, encoding);
        return escPos;
    }

    public synchronized static EscPos getInstance(OutputStream outputStream) throws IOException {
        return getInstance(outputStream,Constant.DEFAULT_ENCODING);
    }
    ...
    public void print(String template, String param) throws IOException {
        PosParam posParam = JSON.parseObject(param, PosParam.class);  //将json字符串转化为PosParam对象

        Map<String, Object> keyMap = posParam.getKeys();
        //List<Map<String, Object>> goodsParam = posParam.getGoods();
        Map<String,List<Map<String, Object>>> goodsParam = posParam.getGoods();
        List<Map<String, Object>> extrasParam = posParam.getExtras();
        ...
        // print detail
        for(String title : goodsParam.keySet()){
            escPos.printStr("-----------" + title + "-----------");
            escPos.line(1);
            for (Map<String, Object> goods : goodsParam.get(title)) {
        ...
        // print detail
        for (Map<String, Object> extras : extrasParam) {
            printGoods(extras, posTpl.getExtras());
        }
     ...
     * 根据模班内容和参数打印二维码联
     *
     * @param qr_content 模板内容
     * @param qr_str    参数
     * @throws IOException
     */
    public void printQR(String qr_content, String qr_str) throws IOException {
        escPos.qrImage(qr_content)
            .align(1)
            .size(2)
            .printStr(qr_str)
            .line(2)
    ...
    private EscPos qrImage(String qrData) throws IOException{
        /*Bitmap bmp = QRCodeUtil.encodeAsBitmap(qrData,240);
        socketOut.write(PrintPicture.draw2PxPoint(bmp));*/
        socketOut.write(PrintPicture.createQRImage(qrData,320, false));
        socketOut.flush();
        return this;
    }
    ...
    private static void printQrCode(QrCode qrCode) throws IOException {
        /*escPos.qrCode(qrCode.getFormat(), qrCode.getText())
                .line(qrCode.getLine());*/
        escPos.align(qrCode.getFormat())
                .qrImage(qrCode.getText())
                .line(qrCode.getLine());
        escPos.init();
    }
