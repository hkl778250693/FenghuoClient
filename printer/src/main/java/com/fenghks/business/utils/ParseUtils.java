...
public class ParseUtils {
    ...
    public static JSONObject parseDataString(String json) {
        JSONObject obj = null;
        try {
    ...
    public static JSONArray parseDataArray(String json) {
        JSONObject obj = null;
        try {
            obj = new JSONObject(json);
            if (0 == obj.optInt("status")) {
    ...
    /**
     * 解析打印机实体类  （这里云打印机只需要解析添加网络打印机），所以类型直接传的 1 （网络打印机）
     *
     * @param obj
     * @return
     */
    public static Printer parsePrinter(Context context, JSONObject obj) {
        Printer item = new Printer(SPUtil.getInt(context, AppConstants.APP_SHOP_ID), obj.optString("ip"), obj.optInt("port"), obj.optString("name"),
                obj.optInt("businessNum"), obj.optInt("kitchenNum"), obj.optInt("customerNum"),
                obj.optInt("senderNum"), obj.optInt("qrcode"), obj.optInt("ispause"), AppConstants.PRINTER_TYPE_NETWORK, AppConstants.STATUS_NORMAL);
        return item;
    }
    ...
    public static Order parseOrder(JSONObject obj) {
        Order order = new Order(obj.optInt("orderid"), obj.optInt("areaid"), obj.optInt("businessid"), obj.optString("businessName", ""),
